...
class CommittedMetadataTests(unittest.TestCase):
    def test_exact_original_metadata_and_source_qualification(self):
        ...
        self.assertEqual((lock['expectedFiles'], lock['expectedBytes']), (1146, 595553898))
        self.assertEqual([r['version'] for r in lock['releases']], ['v0.111.1'])
        self.assertEqual(inventory['base'], 'https://mekhovov.github.io/revealline-archive-77/')
        release = lock['releases'][0]
        self.assertEqual(release['tagObject'], 'f352058fa29245e0da52784600f0f71582f2e5f2')
        self.assertEqual(release['sourceRevision'], 'cd9565357add4d1521faae76e6d029446b6d8503')
        self.assertEqual(release['sourceTree'], '48f4fb7fbfd8355771eafbe2bd8db80f0a2840b6')
        self.assertEqual(release['sourceQualification'], {'bytes': 14428, 'sha256': '5d131d1a8767b9c0325571e4b88624f9ea5e1391e8f5c8718f2f587ddf93737d', 'policyEvidence': {'path': 'publishing/test-policy.json', 'bytes': 490, 'sha256': 'b6887ba7f2b84a007b96de14fc867ac38ac8135d7c231c7ade7ebc52ba31704a'}})
        cohort = [r for r in inventory['files'] if r['path'].startswith('releases/v0.111.1/')]
        self.assertEqual((len(cohort), sum(r['bytes'] for r in cohort)), (1143, 595552501))
